...
class FileMetadataGenerator:
    ...
    def get_existing_metadata(self, file_id: str) -> Dict[str, Any]:
    ...
    def process_file_metadata(self, file_id: str, user_id: str, file_path: str) -> Dict[str, Any]:
        """Main method to process file and generate enhanced metadata"""
        try:
            logger.info(f"Starting metadata processing for file_id: {file_id}")
            
            # Get existing metadata
            existing_metadata = self.get_existing_metadata(file_id)
            ...
            if self.is_metadata_updated(existing_metadata):
                logger.info(f"Metadata for file {file_id} is already updated")
...
def check_metadata_status(file_id):
    ...
    try:
        metadata = metadata_generator.get_existing_metadata(file_id)
